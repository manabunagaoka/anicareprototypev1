  ...
  const sendToOpenAI = async (text: string, isVoice: boolean = false) => {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ 
          message: text,
          context: 'You are AniCare, a casual chat companion. Keep responses brief and friendly.'
        })
      });
            ...
            {chatMessages.map((message, index) => (
              <div key={index} className="text-sm break-words">
                <span className="font-medium text-gray-600">
                  {message.sender === 'user' ? 'You' : 'AniCare'}:
                </span>
                <span className="text-gray-800 ml-1">{message.text}</span>
              </div>
            ))}
            {isProcessing && (
              <div className="text-sm text-gray-500 italic">AniCare is typing...</div>
            )}
